...
abstract class query_builder extends DB_driver {
	...
	protected function _max_min_avg_sum($select = '', $alias = '', $type = 'MAX')
	{
		...
		$sql = $type.'('.trim($select).') AS '.trim($alias);
	...
	protected function _wh($qb_key, $key, $value = NULL, $type = 'AND ')
	{
		...
		foreach ($key as $k => $v)
		{
			...
			${$qb_key} = array('condition' => $prefix.$k, 'value' => $v);
			$this->{$qb_key}[] = ${$qb_key};
		}
	...
	protected function _like($field, $match = '', $type = 'AND ', $side = 'both', $not = '')
	{
		...
		foreach ($field as $k => $v)
		{
			...
			$qb_where = array('condition' => "{$prefix} {$k} {$not} LIKE {$v}", 'value' => NULL);
	...
	{
		return $this->_wh('qb_having', $key, $value, 'AND ');
	}
	...
	public function count_all_results($table = '', $reset = TRUE)
	{
		...
		$result = ($this->qb_distinct === TRUE OR ! empty($this->qb_groupby) OR $this->qb_limit OR $this->qb_offset)
			? $this->query($this->_count_string.'numrows'."\nFROM (\n".$this->_compile_select()."\n) CI_count_all_results")
			: $this->query($this->_compile_select($this->_count_string.'numrows'));
	 ...
	 */
	public function insert_batch($table, $set = NULL, $batch_size = 100)
	{
		if ($set === NULL)
		{
		...
		{
			if ($this->query($this->_insert_batch($table, $this->qb_keys, array_slice($this->qb_set, $i, $batch_size))))
			{
	 ...
	 * @return	CI_DB_query_builder
	 */
	public function set_insert_batch($key, $value = '')
	{
	...
	public function update_batch($table, $set = NULL, $index = NULL, $batch_size = 100)
	{
		...
		{
			if ($this->query($this->_update_batch($table, array_slice($this->qb_set_ub, $i, $batch_size), $index)))
			{
				$affected_rows += $this->affected_rows();
			}
	 ...
	 * @return	CI_DB_query_builder
	 */
	public function set_update_batch($key, $index = '')
	{
		$key = $this->_object_to_array_batch($key);
		...
		if (count($this->$qb_key) > 0)
		{
			for ($i = 0, $c = count($this->$qb_key); $i < $c; $i++)
			{
				...
				for ($ci = 0, $cc = count($conditions); $ci < $cc; $ci++)
				{
					...
					if ( ! empty($matches[4]))
					{
						$this->_is_literal($matches[4]) OR $matches[4] = trim($matches[4]);
						$matches[4] = ' '.$matches[4];
					}
